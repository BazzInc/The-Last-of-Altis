...
if((_vInfo select 1) == "med" && (_vInfo select 2) in ["C_Offroad_01_F","C_SUV_01_F","C_Van_01_box_F","B_Truck_01_mover_F"]) then
{
	[[_vehicle,"med_offroad",true],"life_fnc_vehicleAnimate",_unit,false] spawn life_fnc_MP;
};
if((_vInfo select 1) == "adac"  && (_vInfo select 2) in ["C_Offroad_01_repair_F","C_SUV_01_F"]) then
{
